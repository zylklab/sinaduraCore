...
/**
 * Basada en PropsTruster (cambio de properties a keystore)
 *
 */
public class KeystoreTruster extends TrustAdapter {

	/** Logger. */
	private static final Log LOG = LogFactory.getLog(KeystoreTruster.class);
	/** Internacionalizador. */
     ...
     */
    public KeystoreTruster(KeyStore keystore) {
        loadConf(keystore);
    }
    
	protected synchronized void loadConf(KeyStore keystore) {
		
		if (keystore != null) {
			try {
				CertificateFactory cf = CertificateFactory.getInstance("X509");
				issuersCerts = loadCerts(cf, keystore, TrusterType.TRUSTER_SIGNCERTS_ISSUER);
				certsCerts = loadCerts(cf, keystore, TrusterType.TRUSTER_SIGNCERTS_CERTS);
				issuersOCSP = loadCerts(cf, keystore, TrusterType.TRUSTER_OCSP_ISSUER);
				certsOCSP = loadCerts(cf, keystore, TrusterType.TRUSTER_OCSP_CERTS);
				issuersCRL = loadCerts(cf, keystore, TrusterType.TRUSTER_CRL_ISSUER);
				issuersTSA = loadCerts(cf, keystore, TrusterType.TRUSTER_TSA_ISSUER);
				certsTSA = loadCerts(cf, keystore, TrusterType.TRUSTER_TSA_CERTS);
	...
	public void isTrusted(final OCSPResp ocsp) throws TrustException {
		...
		if (issuersOCSP != null)  {
			...
			try {
				// TODO: corregir el provider en función del tipo de firma de la respuesta
				if (basicResp.verify(certs[0].getPublicKey(), "SunRsaSign")) { // Y si no es firma RSA?
					return;
				} else {
					throw new FakedTrustException();
				}
			...
			if (certsCollection.size() > 0) {
				...
				while (it.hasNext()) {
					...
					try {
						...
						LOG.error(I18N.getLocalMessage(ConstantsTrust.I18N_TRUST_PROPS_16, ex.getMessage()));
						if (LOG.isDebugEnabled()) {
							LOG.debug("", ex);
						}
						throw new UnknownTrustException();
					} catch (NoSuchAlgorithmException ex) {
						LOG.error(I18N.getLocalMessage(ConstantsTrust.I18N_TRUST_PROPS_17, ex.getMessage()));
						if (LOG.isDebugEnabled()) {
							LOG.debug("", ex);
						}
						throw new UnknownTrustException();
	...
	// TODO como en nuestro caso el ks viene desde el cliente no se puede implementar un singleton de la misma manera.
...
//		if (instance == null) {
//			instance = new KeystoreTruster(CONF_DEFAULT, null);
//		}
//		return instance;
//	}
	 ...
	 */
	private CertStore loadCerts(final CertificateFactory cf, KeyStore keystore, final TrusterType trusterType) {
	...
	public CertPath getCertPath(X509Certificate cert) throws UnknownTrustException {
		...
		if (!chainCompleted) {
			throw new UnknownTrustException(I18N.getLocalMessage(ConstantsTrust.I18N_TRUST_PROPS_23) + cert.getSubjectX500Principal().getName());
		}
