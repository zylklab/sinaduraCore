 ...
 */
package net.esle.sinadura.core.interpreter;
...
import net.esle.sinadura.core.exceptions.InterpreterInterruptedException;
...
public class ValidationInterpreterUtil {
	...
	public static SignatureInfo parseResultadoValidacion(XadesSignatureInfo result) {
		...
		// fecha
		Date fecha = null;
		if (result.getDatosFirma() != null && result.getDatosFirma().getDatosSelloTiempo() != null
				&& result.getDatosFirma().getDatosSelloTiempo().size() > 0) {
			// fecha del sello de tiempo
			fecha = result.getDatosFirma().getDatosSelloTiempo().get(0).getFecha();
		} else if (result.getDatosFirma() != null && result.getDatosFirma().getFechaFirma() != null) {
			// fecha de firma
			fecha = result.getDatosFirma().getFechaFirma();	
		}
		
		signatureInfo.setDate(fecha);
		...
		} else if (result.getDatosFirma() != null && result.getDatosFirma().getCadenaFirma() != null && result.getDatosFirma().getCadenaFirma().getCertificates() != null) {
		...
		// tsChain
		if (result.getTsChain() != null) {
			signatureInfo.setTsChain(result.getTsChain());
		}
		...
		try {
			...
			if (!result.isValidate()) {
				...
				messageInfo.setSimpleStatus(Status.VALID);
				String m = MessageFormat.format(LanguageUtil.getLanguage().getString("info.validation.signature.type"),
						result.getDatosFirma().getTipoFirma().getTipoXAdES().toString());
				messageInfo.setText(m);
				messages.add(messageInfo);
				...
				if (result.getEnumNivel().equals(EnumFormatoFirma.XMLSignature) || result.getEnumNivel().equals(EnumFormatoFirma.XAdES_BES)) {
					...
					messageInfo = new MessageInfo();
					messageInfo.setSimpleStatus(Status.VALID_WARNING);
				...
				if (result.getDatosFirma().esCadenaConfianza().equals(ConfianzaEnum.CON_CONFIANZA)) {
					...
					signatureInfo.setStatus(Status.VALID_WARNING); // baja a warning
					messageInfo = new MessageInfo(Status.VALID_WARNING, LanguageUtil.getLanguage().getString("info.validation.chain.untrust"));
					messages.add(messageInfo);
				}
	...
	public static SignatureInfo parsePdfSignatureInfo(PDFSignatureInfo pdfSignatureInfo) {
		...
		SignatureInfo signatureInfo = new SignatureInfo();
		if (pdfSignatureInfo.getChainInfo() != null) {
			signatureInfo.setChain(pdfSignatureInfo.getChainInfo().getChain());
		}
		signatureInfo.setDate(pdfSignatureInfo.getDate());
		
		if (pdfSignatureInfo.getTimestampInfo() != null && pdfSignatureInfo.getTimestampInfo().getChainInfo() != null) { 
			signatureInfo.setTsChain(pdfSignatureInfo.getTimestampInfo().getChainInfo().getChain());
		}
	...
	private static MessageInfo getDateMessage(PDFSignatureInfo pdfSignatureInfo) {
		...
		} else if (timestampInfo.getStatus().equals(Status.UNKNOWN)) {

			messageInfo.setSimpleStatus(Status.UNKNOWN);
			// TODO dejo el mismo mensaje. habria que parsear el error
			// chain incompleta o revocacion desconocida
			String m = LanguageUtil.getLanguage().getString("info.validation.date.timestamp.unknown");
			messageInfo.setText(m);
			
		} else if (timestampInfo.getStatus().equals(Status.VALID_WARNING)) {

			messageInfo.setSimpleStatus(Status.VALID_WARNING);
			// TODO dejo el mismo mensaje. habria que parsear el error
			// probablemente untrust
			String m = LanguageUtil.getLanguage().getString("info.validation.date.timestamp.unknown");
			messageInfo.setText(m);
